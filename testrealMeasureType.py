...
columns = ['NCT Number', 'Unit of Measure']
...
for nct_number in prostate_nct_numbers:
    ...
    for outcome in other_events:
        ...
        if outcomeMeasureType == "PRIMARY":
            ...
            if categorize_string(measure_title)=='ORR':  # Check if measure_title is not empty
...
# 결과를 DataFrame으로 변환
results_df = pd.DataFrame(list(unit_of_measure_count.items()), columns=['Unit of Measure', 'Count'])
results_df = results_df.sort_values(by='Count', ascending=False)
results_df.to_csv('Unit_of_Measure_Count_ORR.csv', index=False)
...
#                                 measure_values[measure_title].append(float(value))
...
# results_df.to_csv('Breast_Measure_Counts_Statistics_DFS.csv', index=False)
